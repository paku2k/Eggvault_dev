...
const char *keyManualFlag = "key_manual_flag"; 
...
void timeUpdate()
{
  ...
  rtc.updateTime();
  //rtc.writeBit(RV8803_CONTROL, CONTROL_RESET, (uint8_t)0);
  byte control_reg = rtc.readRegister(RV8803_CONTROL);
  //Serial.println(control_reg, BIN);
  byte flag_reg = rtc.readRegister(RV8803_FLAG);
  //Serial.println(flag_reg, BIN);
  byte RAM_reg = rtc.readRegister(RV8803_RAM);
  //Serial.println(RAM_reg, BIN);
...
void menuFunctions(BTNAction action) // Ihre Menüfunktionen
{
  ...
  if (menu_id == 000)
  {
    ...
    if(manualFlag) // Zeige dass manueller modus aktiv ist wenn manualFlag = 1
    {

      lcd.setCursor(0, 0);
      lcd.print(MenuItemsSpecial[5][lang]);
      lcd.setCursor(0, 1);
      lcd.print(MenuItemsSpecial[6][lang]);
      lcd.noBlink();
      lcd.noCursor();
  ...
  if (LDRFlag == POS_DOWN)
  {
    ...
    if (getLux() <= nextLux)
    {
      ...
      { // Counter voll
        ramCounter = 0;
        moveMotor(POS_DOWN);
        return 0; // Damit bei einem Error nicht der Tiefschlaf getriggert wird
      }
      else
...
void setNextOpeningAlarm()
{
  ...
  case ZEIT:
    if (!manualFlag)
    {
      if ((openingAlarms[weekday(t_now) - 1].hour * 60 + openingAlarms[weekday(t_now) - 1].minute) > (hour(t_now) * 60 + minute(t_now))) // Alarm noch nicht vergangen
      {
  ...
  case LICHT:
    ...
    if (!manualFlag)
    {
      ...
      ramCounter = 0; // Debounce counter wird zurückgesetzt, falls er bisher nicht zurückgesetzt wurde
  ...
  default:
    ...
    {
      setAlarmTomorrow0();
      errorFlag = WRONG_ALARM_MODE; // kann eigentlich nicht passieren
...
void setNextClosingAlarm()
{
  ...
  {
  case ZEIT:
    if ((closingAlarms[weekday(t_now) - 1].hour * 60 + closingAlarms[weekday(t_now) - 1].minute) > (hour(t_now) * 60 + minute(t_now))) // Alarm noch nicht vergangen
    {
      alarmFlag = nextMove;
...
void moveMotor(KLAPPENPOSITION pos)
{
  ...
  */
  ...
  while (doorPosition != pos)
  {
    ...
    if(digitalRead(SW_SELECT) == HIGH || digitalRead(SW_EXIT) == HIGH || digitalRead(SW_BACK) == HIGH || digitalRead(SW_FWD) == HIGH){
      break; // moveMotor wurde manuell unterbrochen
    }

    //=====================END WHILE================
  }

  if (doorPosition != POS_BLOCKED && (errorFlag == TIMER_BLOCKED_ELAPSED || errorFlag == TIMER_OPEN_ELAPSED || errorFlag == TIMER_CLOSE_ELAPSED))
  {
    errorFlag = NO_ERROR;
  }
...
void readValuesFromFlash()
{
  ...
  else
  {
    ...
    //memory.getBytes(keyManualFlag, &manualFlag, sizeof(manualFlag));
...
void wakeupHandling()
{
  ...
  case ESP_SLEEP_WAKEUP_EXT1:
    ...
    if(digitalRead(LED)==1){
      manualFlag = 1; //TODO: Testen ob das klappt
    }
    switch (w_pin)
    {
    case CLK_INT:
    ...
    case SW_BACK:
      ...
      {
        //Serial.println("Klappe wurde manuell geschlossen. Bewege Motor abwärts...");
        manualFlag = 1;
        menuActive = 0;
        moveMotor(POS_DOWN);
      }
...
void loop()
{
  ...
  { // Alarm ist während der Menülaufzeit gekommen //TODO: Was wenn alarm auf 00:01 gestellt wurde? --> verhindern oder Fehlerreaktion (immer auf 00:02 aufrunden zb)
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print(MenuItemsSpecial[0][lang]); // Klappe ist beschäftigt, menu wird unterbrochen
    alarmWakeupHandling();
  }
  else
  {
