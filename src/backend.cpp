...
int16_t setNextAlarm(bool tomorrow)
{
  DoW = Clock.getDoW();
  int now = Clock.getHour(h12Flag, pmFlag) * 60 + Clock.getMinute();

  int oA = openingAlarms[DoW].hour * 60 + openingAlarms[DoW].minute;
  int cA = closingAlarms[DoW].hour * 60 + closingAlarms[DoW].minute;
  ...
  else
  {
    if (oA < now && cA < now)
    {
      //Beide schon vergangen
      setNextAlarm(true);
    }
    else if (oA > now && cA < now)
    {
      //Schon geschlossen
      setAlarm(openingAlarms[DoW]);
    }
    else if (oA < now && cA > now)
    {
      //Schon geöffnet
      setAlarm(closingAlarms[DoW]);
    }
    else if (oA > now && cA > now)
    {
      //Keiner Vergangen
      ...
      //Beide sind genau jetzt, erst wurde geschlossen
      setAlarm(openingAlarms[DoW]);
    }
  }
}
